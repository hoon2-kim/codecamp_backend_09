    ...
    await Promise.all([
        new Promise((resolve, reject) => {
            ...
            storage
                ...
                .pipe(storage.file(`thumb/s/${event.name}`).createWriteStream()) // 6. 생성된 썸네일을 재업로드
                .on("finish", () => resolve())
                .on("error", () => reject());
        }),
        new Promise((resolve, reject) => {
            ...
            storage
                ...
                .createReadStrea() // 4. 기존의 파일을 읽어오기
                .pipe(sharp().resize({ width: 320 })) // 5. event 안에 있는 file을 활용하여 썸네일 생성
                .pipe(storage.file(`thumb/m/${event.name}`).createWriteStream()) // 6. 생성된 썸네일을 재업로드
                .on("finish", () => resolve())
                .on("error", () => reject());
        }),
        new Promise((resolve, reject) => {
            ...
            storage
                .file(event.name)
                .createReadStrea() // 4. 기존의 파일을 읽어오기
                .pipe(sharp().resize({ width: 1280 })) // 5. event 안에 있는 file을 활용하여 썸네일 생성
                .pipe(storage.file(`thumb/l/${event.name}`).createWriteStream()) // 6. 생성된 썸네일을 재업로드
                .on("finish", () => resolve())
                .on("error", () => reject());
        }),
    ]);

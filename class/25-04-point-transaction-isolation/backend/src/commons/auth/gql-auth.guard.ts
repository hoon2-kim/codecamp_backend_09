...
export class GqlAuthAccessGuard extends AuthGuard('access') {
  ...
  getRequest(context: ExecutionContext) {
    const gqlContext = GqlExecutionContext.create(context); // rest-api용 context를 gql용으로 다시 만듬
    return gqlContext.getContext().req; // req정보만 뽑아서 GqlAuthAccessGuard로 리턴
  }
}
...
// graphql에서는 여기서 검증을 하고 완료되면 AuthGuard 검증함

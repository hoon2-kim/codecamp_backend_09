...
export class UsersResolver {
  ...
  // commons/auth/jwt-access.strategy.ts(인가 관련로직)
  @UseGuards(GqlAuthAccessGuard) // 여기서 인가라는 것이 이루어지고 통과해야 밑에 fetchUser가 이루어짐
  @Query(() => String)
  fetchUser(
    @Context() context: IContext, // graphql / req,res가 들어있음
  ) {
    // 유저 정보 꺼내오기

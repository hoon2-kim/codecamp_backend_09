...
exports.generateThumbnail = async (event, context) => {
    ...
    // event.name // 파일이름이 이렇게 들어옴 2022/01/13/sdgjio-dsgsdjgp-123-sdgsdg/origin/강아지.jpg
    // 여기선 origin을 바꿔야함
    // origin 앞 부분을 prefix , 뒷 부분을 postfix로 나눈다
            ...
            (el) =>
                new Promise((resolve, reject) => {
                    ...
                    storage
                        ...
                        .pipe(storage.file(`${el.fname}`).createWriteStream()) // 6. 생성된 썸네일을 재업로드
                        .on("finish", () => resolve())
                        .on("error", () => reject());
                })
        )

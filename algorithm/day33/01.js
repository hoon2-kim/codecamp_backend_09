...
function solution(new_id) {
    ...
    // 2단계
    const replace = lowerId.replace(/[^a-z0-9-_.]/g, "");
    // 3단계
    for (let i = 0; i < replace.length; i++) {
        for (let j = i + 1; j < replace.length + 1; j++) {
...
// 멘토님
const permission = "qwertyuiopasdfghjklzxcvbnm-_.";
...
function solution(new_id) {
    ...
    // 6단계 : 길이가 16이상이라면, 15의 길이를 만들어주고, 문자열의 마지막에 마침표가 있다면, 다시 제거
    if (answer.length >= 16) {
        answer = answer.substring(0, 15);
        removeDot();
    }

    // 7단계 : 문자열의 길이가 2 이하라면, 길이가 3일 될 때까지 마지막 글자를 반복해서 추가
    if (answer.length <= 2) {
...
function solution(new_id) {
    ...
    // 7단계 : 문자열의 길이가 2 이하라면, 길이가 3일 될 때까지 마지막 글자를 반복해서 추가
    if (answer.length <= 2) {
        const add = new Array(3 - answer.length).fill(
